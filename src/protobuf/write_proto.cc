...
bool write_batch_data(Batch &batch_data, const std::string& file_name) {

  LOG(INFO) << "Begin write batch data ...";

  google::uint32 fea_type_nums = batch_data.fea_types.size();
  google::uint32 value_rows = batch_data.samples.size();
  google::uint32 value_cols = 0;
  if(value_rows > 0){
  ...
  for (auto fea_type : batch_data.fea_types) {
  	...
  	// data::FeaType* new_fea_type = write_batch_data.add_fea_types();
  	// (* new_fea_type) = batch_data.fea_types[i];
  }

  for (auto sample_row : batch_data.samples) {
  	for (size_t i = 0; i < sample_row.size(); ++i) {
  	//for (auto sample : sample_row) {
  		// data::Value *new_value = write_batch_data.add_samples();
  		LOG(INFO) << batch_data.fea_types[i] ;
  		if (batch_data.fea_types[i] == CONT){
  			write_batch_data.add_samples()->set_v(sample_row[i].v);
  			//new_value->set_v(float(sample_row[i]));
  		} else if (batch_data.fea_types[i] == DISC) {
  			write_batch_data.add_samples()->set_id(sample_row[i].cls);
...
int main(int argc, char const *argv[])
{
  ...
  // for (auto sample : batch_data.samples) {
  // 	for (auto s : sample) {
  // 		LOG(INFO) << s. ;
  // 	}
  // }
